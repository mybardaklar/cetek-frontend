...
class AppService {
	...
	async testHomePageSettings(language) {
		try {
			const request = await axiosInstance.get(
				`/pages/${language === "en" ? 429 : 194}?_fields=acf&acf_format=standard`,
			);

			/* const blob = await put("homePageSettings.json", JSON.stringify({ deneme: "test" }), {
				access: "public",
